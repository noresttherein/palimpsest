...
sealed trait LongSet extends FitSet[Long] with SetSpecialization[Long, LongSet] {
	...
	/** Masked bits common to all elements in this set. For all values in this set `e`, `(e & mask) == path`.
	  ...
	  * In default trie implementation values are stored in a Trie where paths follow the binary representation
	...
	override def fitIterator :FitIterator[Long] = new LongTrieIterator(this)
...
object LongSet {
	...
	sealed trait StableLongTrie extends LongSet with StableSet[Long] with SetSpecialization[Long, StableLongTrie] {
		...
		override def mutable :MutableSet[Long] = new MutableLongSet ++= this
	...
	object Singleton {
		private[LongSet] final val mask = 0xffffffffffffffffL
	}
	...
	{ this :LongSet =>
		...
		private[sets] def right :LongSet
	...
	private type Branch = BranchLike[LongSet] //with LongSet
...
//		private[LongSet] def suffixJoin(s1 :StableLongSet, s2 :StableLongSet) :StableBranch = {
//			val suffix1 = s1.path
...
//		}
				...
				with MutableSet[Long] with MutableSetLike[Long, MutableLongSet] with LongSet
	{
		...
		def filtered(l: LongSet, r: LongSet) :MutableLongSet =
			if (l.isEmpty) r match {
				...
				case _ => throw new MatchError(s"Illegal child of a MutableLongSet - neither a MutableLongSet nor Empty: $l")
			} else if (r.isEmpty)
	...
	/** An iterator traversing a trie using a manual stack implemented using an array.
	  ...
	  */
	private[LongSet] class LongTrieIterator(stack :Array[LongSet], private[this] var top :Int)
		extends BaseIterator[Long] with FitIterator[Long]
	{
		override protected[this] def mySpecialization = Specialized.SpecializedLong

		def this(trie: LongSet) = {
			this(new Array[LongSet](64), 0)
			stack(0) = trie
			descend()
		}
