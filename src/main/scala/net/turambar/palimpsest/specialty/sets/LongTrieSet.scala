...
import net.turambar.palimpsest.specialty.sets.LongTrieSet.{LongTrieSet1, LongTrieSetN, addElement, removeElement}
...
import net.turambar.palimpsest.specialty.tries.{LongPrefixTrie, Trie, TrieLeaf, TrieKeys}
import net.turambar.palimpsest.specialty.tries.LongPrefixTrie.{EmptyLongTrie, FlippedKeyIterator, LongTrieBranch, LongTrieLeaf, MutableLongTrie, OrderedLongLeaf, OrderedLongTrie, commonPath, flipSign}
import net.turambar.palimpsest.specialty.tries.Trie.{LeafValueIterator, MutableTrieRoot, ReplaceLeaf, TrieCombinator, TriePatch}
...
trait LongTrieSet extends StableOrderedSet[Long] with OrderedSetTemplate[Long, LongTrieSet] with SetSpecialization[Long, LongTrieSet]
						  with TrieKeys[Long, Long, LongTrieSet] with TrieSetValues[Long, Long, LongTrieSet] with LongPrefixTrie[Long, LongTrieSet]
{
...
//	override def asTrie :LongTrieSet = this
	...
	override def notFound[S >: LongTrieSet](key: Long, sibling: Trie[Long, Long, S]): S = new LongTrieSet1(key)

	override def updateLeaf[S >: LongTrieSet](oldLeaf: TrieLeaf[Long, Long, S]): S = oldLeaf.asInstanceOf[LongTrieSet]

	override private[sets] def update(root: MutableTrieRoot[LongTrieSet], element: Long, mutant: TriePatch[Long, Long, LongTrieSet]) :Unit =
...
//	override def disjoint(first: LongTrieSet, second: LongTrieSet): LongTrieSet = reduce(first, second)
	...
	override def reduce(first :LongTrieSet, second :LongTrieSet) :LongTrieSet = {
...
object LongTrieSet {
	...
	object Empty extends EmptyLongTrie[Long, Long, LongTrieSet] with LongTrieSet
	...
	class LongTrieSet1(flipped :Long)
		extends LongTrieLeaf[Long, Long, LongTrieSet](flipped) with ValueLeaf[Long, Long, LongTrieSet]
				with LongTrieSet with OrderedLongLeaf[Long, LongTrieSet] with SingletonSpecialization[Long, LongTrieSet] //with TriePatch[Long, Long, LongTrieSet]
	{
	...
	class LongTrieSetNode(path :Long, zeros :LongTrieSet, ones :LongTrieSet)
		extends LongTrieBranch[Long, LongTrieSet](path, zeros, ones)
				with ValueTrieTemplate[Long, Long, LongTrieSet]
				with OrderedLongTrie[Long, LongTrieSet] with LongTrieSet
...
//		override private[sets] def update(root: MutableTrieRoot[LongTrieSet], element: Long, mutant: TriePatch[Long, Long, LongTrieSet]) =
...
//		override def matched(left: LongTrieSet, right: LongTrieSet): LongTrieSet = left.editable
		...
		override def reduce(first: LongTrieSet, second: LongTrieSet) = {
		...
		override def disjoint(first: LongTrieSet, second: LongTrieSet): LongTrieSet = reduce(first.editable, second.editable)
...
//		override def matched(left: LongTrieSet, right: LongTrieSet): LongTrieSet = left
		...
		override def disjoint(first: LongTrieSet, second: LongTrieSet): LongTrieSet = reduce(first, second.stable)
...
//		override def stable = this
	...
	@inline final def addElement :TriePatch[Long, Long, LongTrieSet] = NewLeaf

	private[this] final val NewLeaf = new TriePatch[Long, Long, LongTrieSet] {
		override def notFound[S>:LongTrieSet](key: Long, sibling: Trie[Long, Long, S]): S = new LongTrieSet1(key)
		override def updateLeaf[S>:LongTrieSet](oldLeaf: TrieLeaf[Long, Long, S]): S = oldLeaf.asTrie
	}

	@inline final def removeElement :TriePatch[Long, Long, LongTrieSet] = RemoveLeaf
