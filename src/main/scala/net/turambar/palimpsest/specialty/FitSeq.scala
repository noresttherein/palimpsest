...
/** A scala `Seq` (either mutable or immutable underneath) which is specialized on its element type.
  * In order not to polute the API with a counterpart of every scala collection interface, it rolls `IndexedSeq` and `Seq`
  * into one. Subclasses are expected to provide fast [[FitSeq#size]] operation to benefit from proper buffer allocation
...
trait FitSeq[@specialized(Elements) +E]
	extends IndexedSeq[E] with FitIterable[E] with SpecializedTraversableTemplate[E, FitSeq] with FitSeqLike[E, FitSeq[E]]
{
...
object FitSeq extends InterfaceIterableFactory[FitSeq] {
	...
	override protected[this] final def default: FitIterableFactory[ConstArray] = ConstArray
...
//		fitCBF.cbf
	...
	class Seq1[@specialized(Elements) +E](override val head :E)
		extends IterableFoundation[E, ConstSeq[E]] with ConstSeq[E]
	{
		override def headOption = Some(head)
		override def last = head
		override def lastOption = headOption
		
		override def tail = ConstSeq.Empty
		...
		@unspecialized
		override protected def section(from: Int, until: Int): ConstSeq[E] =
		...
		override def filter(f :E=>Boolean) :ConstSeq[E] =
			if (f(head)) this
			else ConstSeq.Empty
		
		override def filterNot(f :E=>Boolean) :ConstSeq[E] =
			if (f(head)) ConstSeq.Empty
			else this
		
		override def map[@specialized(Fun1Vals) U, That](f: (E) => U)(implicit bf: CanBuildFrom[ConstSeq[E], U, That]): That = bf match {
			case fit :CanFitFrom[_, _, _] if fit.companion == FitSeq || fit.companion == ConstSeq =>
				new Seq1(head).asInstanceOf[That]
			case _ => (bf(this) += f(head)).result()
		}
		
		//todo: check if the builder can't just return mapped sequence
		override def flatMap[U, That](f: (E) => GenTraversableOnce[U])(implicit bf: CanBuildFrom[ConstSeq[E], U, That]): That =
	...
	class Seq2[@specialized(Elements) +E](override val head :E, override val last :E)
		extends IterableFoundation[E, ConstSeq[E]] with ConstSeq[E]
	{
		override def headOption = Some(head)
		...
		override def filter(p: (E) => Boolean): ConstSeq[E] =
			if (p(head))
				if (p(last)) this
				else new Seq1(head)
		...
		override def filterNot(p: (E) => Boolean): ConstSeq[E] =
			if (p(head))
				if (p(last)) ConstSeq.Empty
				else new Seq1(last)
		...
		override def map[@specialized(Fun1Vals) O, That](f: (E) => O)(implicit bf: CanBuildFrom[ConstSeq[E], O, That]): That = bf match {
			case fit :CanFitFrom[_, _, _] if fit.companion==FitSeq || fit.companion == ConstSeq =>
	...
	class ErasedFitSeq[E](private val backing :Seq[E], private val offset :Int, val length :Int)
		extends IterableFoundation[E, FitSeq[E]] with FitSeq[E]
	{
