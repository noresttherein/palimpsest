...
/** A boxed `@volatile` variable of type `T`. Volatile declaration allows it to be used concurrently, guaranteeing
  ...
  */
sealed class Volatile[@specialized(Var.SpecializedTypes) T](init :T) extends InOut[T] with Serializable {
...
object Volatile {
	...
	@inline def apply[@specialized(Var.SpecializedTypes) T](value :T) :Volatile[T] = new Volatile(value)

	@inline implicit def unboxVar[@specialized(Var.SpecializedTypes) T](vol :Volatile[T]) :T = vol.get
