...
import Text.Constants._

/** Interface declaring methods mirroring those of [[scala.collection.immutable.StringLike]], but working on [[Text]]s.
  * [[Text]] can't extend `StringLike` because that one extends [[Ordered[String]], while we want ordering methods
  * working on [[Text]] instances first and foremost. While I can imagine that a workaround is possible,
  * it is an implementation rather than API interface anyway and almost no code could be effectively reused.
  *
  * @author Marcin Mościcki
  */
trait AlmostStringLike { this :Text =>
	...
	*/
	def stripLineEnd: Text = {
		val len = toString.length
		if (len == 0) toString
		else {
			val last = this.last
			if (isLineBreak(last))
				subtext(0, if (last == LF && len >= 2 && apply(len - 2) == CR) len - 2 else len - 1)
			else
				this
		}
	}
