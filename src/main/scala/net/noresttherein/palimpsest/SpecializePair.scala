...
import net.noresttherein.palimpsest.SpecializePair.{Curry, SpecializeFirst}


/** Similarly to how [[net.noresttherein.palimpsest.Specialize]] performs a callback to a specialized method
...
abstract class SpecializePair[R[X, Y]] extends Specialize[SpecializeFirst[R]#T] { pair =>
	...
	/** Initiates the double callback using the implicitly passed runtime type information to pick the appropriate
	  * specialized variant of [[net.noresttherein.palimpsest.SpecializePair#specialize[X, Y]].
	  */
	def apply[X, Y]()(implicit xType :RuntimeType[X], yType :RuntimeType[Y]) :R[X, Y] = apply[X]().apply[Y]()

	/** Intermediate callback specialized for the first type parameter, which returns the callback for the specialization
	  * of the second parameter.
	  */
	override def specialized[@specialized X :RuntimeType] :Specialize[Curry[R]#T1[X]#T2] =
		new SpecializeSecond[X]

	/** Specialized callback method invoked as the result of the call to `apply()`, responsible for producing the final result value. */
...
object SpecializePair {
	...
	/** An equivalent of [[net.noresttherein.palimpsest.Specialize.SpecializeSome]], it allows extending classes
	  ...
	  * @tparam R a type constructor for the result type, accepting two specialized type parameters.
	  */
	abstract class SpecializeSomePairs[R[X, Y]] extends SpecializePair[R] with SpecializeIndividually[SpecializeFirst[R]#T] {
		...
		/** Method invoked to initialize individual `forT` fields, having the specialization of the first type parameter.
		  ...
		  * Default callback results in eventual invokation of this instance's `specialized` method.
		  */
		...
		/** The final target of the triple dispatch invoked when the specialization for both types has been determined.
		  * Note that overriding this method will create `100` synthetic variants! consider overriding individaul
		  * `for`''T'' methods and [[net.noresttherein.palimpsest.SpecializePair.SpecializeSomePairs]] instead.
		  * @return result of invoking `generic`
		  */
		override def specialized[@specialized X :RuntimeType, @specialized Y :RuntimeType] = generic[X, Y]
