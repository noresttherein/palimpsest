...
import net.noresttherein.palimpsest.seqs.AptList
import net.noresttherein.palimpsest
import net.noresttherein.palimpsest._
import net.noresttherein.palimpsest.iterables.AptIterable.{FilterIterable, SpecializedFilter}
...
import net.noresttherein.palimpsest.seqs.{AptBuffer, AptList, AptSeq, ArrayView}
...
/** Partial specialization of the `IterableLike` trait containing methods which can be implemented
  ...
  * @see [[net.noresttherein.palimpsest.iterables.IterableFoundation]] - unspecialized base class implementing most methods
...
//	@deprecated("what does it even mean to inverse a set...", "")
	...
	/** Returns an independent copy of this iterable. It will avoid copying as much of the contents as possible,
	  ...
	  * @see [[net.noresttherein.palimpsest.iterables.IterableTemplate#clone]]
	  */
	def carbon :Repr
...
trait CloneableIterable[+E, +Repr <: AnyRef] extends IterableTemplate[E, Repr] {
	...
	/** Creates a deep copy of this iterable. Not only any future modifications to one will have no effect on the other,
	  ...
	  * @see [[net.noresttherein.palimpsest.iterables.IterableTemplate#carbon]]
	  */
...
//	override protected[this] def forLast[@specialized(Fun1Res) O](f: E => O) :O = f(last)
	...
	override def span(p: E => Boolean): (Repr, Repr) = palimpsest.span(iterator, newBuilder, newBuilder)(p)

	override def filter(p :E => Boolean, neoTruth :Boolean) :Repr = palimpsest.filter(iterator, newBuilder)(p, neoTruth)
	
	override def withFilter(p: E => Boolean): FilterIterable[E, Repr] = new SpecializedFilter(this, p)

	override def partition(p: E => Boolean): (Repr, Repr) = palimpsest.partition(iterator, newBuilder, newBuilder)(p)
