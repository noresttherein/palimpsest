...
import net.noresttherein.palimpsest.{?, ItemTypes, Sure}
import net.noresttherein.palimpsest.iterators.AptIterator
import net.noresttherein.palimpsest.tries.BinaryTrie._
import net.noresttherein.palimpsest.tries.EmptyTrie.EmptyTrieFoundation
import net.noresttherein.palimpsest.tries.LongTrieKeys.LongKeyBranch
import net.noresttherein.palimpsest.tries.Trie._
import net.noresttherein.palimpsest.tries.TrieElements.ElementOf
import net.noresttherein.palimpsest.tries.TrieFriends.{EmptyFriend, LeafFriend, TrieOp, TriePredicate}
import net.noresttherein.palimpsest.tries.TrieLeaf.TrieLeafFoundation
...
/** A binary (that is with all inner nodes having arity of `2`) [[Trie]] implementation with `Long` values
  ...
  * any branch instances ([[net.noresttherein.palimpsest.tries.LongTrieKeys.LongKeyBranch]])
...
{ this :This with LongTrie with BinaryTrieNode =>
	...
	/** Analogue of [[net.noresttherein.palimpsest.sets.OrderedSet#from]] working on keys in this trie. Provides a basis for subclasses to implement
	  * [[net.noresttherein.palimpsest.ordered.OrderedVals]], but doesn't extend the trait itself here in order to allow the subclasses to
	...
	/** Analogue of [[net.noresttherein.palimpsest.sets.OrderedSet#until]] working on keys in this trie. Provides a basis for subclasses to implement
	  * [[net.noresttherein.palimpsest.ordered.OrderedVals]], but doesn't extend the trait itself here in order to allow the subclasses to
	  * provide the type parameter for the 'key' argument themselves, possibly different from `Long` (but likely with homomorphic ordering).
	...
	/** Analogue of [[net.noresttherein.palimpsest.sets.OrderedSet#to]] working on keys in this trie. Provides a basis for subclasses to implement
	  * [[net.noresttherein.palimpsest.ordered.OrderedVals]], but doesn't extend the trait itself here in order to allow the subclasses to
	  * provide the type parameter for the 'key' argument themselves, possibly different from `Long` (but likely with homomorphic ordering).
	...
	/** Analogue of [[net.noresttherein.palimpsest.sets.OrderedSet#range]] working on keys in this trie. Provides a basis for subclasses to implement
	  * [[net.noresttherein.palimpsest.ordered.OrderedVals]], but doesn't extend the trait itself here in order to allow the subclasses to
	  * provide the type parameter for the 'key' argument themselves, possibly different from `Long` (but likely with homomorphic ordering).
	  * @param start lower bound (inclusive) on the keys to select from this trie as a new trie.
	  * @param end upper bound (exclusive) on the keys to select from this trie as a new trie.
	...
	/** Analogue of [[net.noresttherein.palimpsest.sets.OrderedSet#rangeImpl]] working on keys in this trie. Provides a basis for subclasses to implement
	  * [[net.noresttherein.palimpsest.ordered.OrderedVals]], but doesn't extend the trait itself here in order to allow the subclasses to
	  * provide the type parameter for the 'key' argument themselves, possibly different from `Long` (but likely with homomorphic ordering).
	...
	/** Primary target method of [[net.noresttherein.palimpsest.tries.TrieFriends.juxtapose]]. Recursively descends
	...
	/** An implementation method used by [[net.noresttherein.palimpsest.tries.TrieFriends.juxtapose]]. Invoked when
