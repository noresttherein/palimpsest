...
import net.noresttherein.palimpsest.tries.Trie.{FoldPath, KeyTypes, TrieOpRes}
import net.noresttherein.palimpsest.tries.TrieFriends.{TrieOp, TriePredicate}
...
trait TrieFriends[/*@specialized(KeyTypes) */K, Friend <: Trie[K, Friend], +This <: Trie[K, This] with Friend] extends TrieTemplate[K, This] {
	...
	/** A variant of [[net.noresttherein.palimpsest.tries.TrieFriends#juxtapose]] which implements a relation
...
object TrieFriends {
	...
	/** An operator returning values of `O` for two tries of type `T` which can be implemented by a divide and conquer
	  ...
	  *  @define Trie net.noresttherein.palimpsest.tries.Trie
	  */
